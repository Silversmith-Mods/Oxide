...
public class VarnishSprayer extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (level.getFluidState(pos).is(FluidTags.WATER) && isWater(context.getItemInHand())) {
            level.playSound(player, player.getX(), player.getY(), player.getZ(), SoundEvents.BOTTLE_FILL, SoundSource.NEUTRAL, 1.0F, 1.0F);
            level.gameEvent(player, GameEvent.FLUID_PICKUP, pos);
            this.setAmount(stack, 1000);
            ...
            if (!level.isClientSide) {
                for(int y = -4; y < 4; ++y) {
                    for(int x = -4; x < 4; ++x) {
                        ...
                        entity.shootFromRotation(player, player.getXRot() + (y * 5 * level.random.nextFloat()), player.getYRot() + (x * 5 * level.random.nextFloat()), 2.0F, 1.5F, 1.0F);
